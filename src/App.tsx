...
export default function App() {
  ...
  const backupData = async () => {
    ...
    const link = document.createElement('a');
    link.href = url;
    link.download = `academic_diary_backup_${new Date().toISOString().split('T')[0]}.json`;
    link.click();
  ...
  const restoreData = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (event) => {
      try {
        ...
        // Clear current DB (optional, but safer to overwrite)
        // We'll just put everything
        for (const cat of backup.categories) await db.saveCategory(cat);
        for (const act of backup.activities) await db.saveActivity(act);
        if (backup.profile) await db.saveProfile(backup.profile);
    ...
    <div className="min-h-screen bg-zinc-50 text-zinc-900 font-sans flex flex-col md:flex-row overflow-x-hidden">
      ...
      <main className="flex-1 flex flex-col min-h-screen">
        ...
        <div className="flex-1 p-6 max-w-5xl mx-auto w-full">
          {view === 'list' && (
            <div className="mb-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 text-[10px] font-bold uppercase tracking-widest text-zinc-400 bg-zinc-100/50 px-3 py-2 rounded-lg border border-zinc-200/50">
              <div className="flex items-center gap-2">
                <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />
                Local Storage Active • Data persists on this device
              </div>
              <div className="flex items-center gap-4">
                <button onClick={backupData} className="hover:text-zinc-900 transition-colors flex items-center gap-1">
                  <Download className="w-3 h-3" />
                  Backup
                </button>
                <label className="hover:text-zinc-900 transition-colors flex items-center gap-1 cursor-pointer">
                  ...
                  <div className="grid gap-4">
                    {activities.map(activity => (
                      <div 
                        ...
                        className="bg-white border border-zinc-200 rounded-xl p-5 hover:shadow-md transition-all group cursor-pointer active:scale-[0.98] relative"
                      >
                        <div className="flex items-start justify-between mb-3">
                          <div className="flex-1">
                            ...
                            <h3 className="text-lg font-bold leading-tight group-hover:text-zinc-600 transition-colors">{activity.title}</h3>
                          </div>
                          <div className="flex items-center gap-2" onClick={e => e.stopPropagation()}>
                            <button 
                              onClick={() => exportToPDF([activity])}
                              className="p-3 hover:bg-zinc-100 rounded-full text-zinc-500 bg-zinc-50 md:bg-transparent"
                              title="Export to PDF"
                            >
                              <FileDown className="w-5 h-5" />
                            </button>
                            ...
                            <button 
                              ...
                              className="p-3 hover:bg-red-50 rounded-full text-red-500 bg-red-50/50 md:bg-transparent"
                              title="Delete"
                            >
